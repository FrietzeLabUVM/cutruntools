...
* `../macs2.narrow.all.frag.aug18`. MACS2, narrowPeak, all fragments, with duplicates.
* `../macs2.broad.all.frag.aug18`. MACS2, broadPeak, all fragments, with duplicates.
* `../macs2.narrow.aug18.dedup`. MACS2, narrowPeak, all fragments, removing duplicates.
* `../macs2.broad.aug18.dedup`. MACS2, broadPeak, all fragments, removing duplicates.
* `../macs2.narrow.all.frag.aug18.dedup`. MACS2, narrowPeak, all fragments, removing duplicates.
...
Use the former one if you wish to keep duplicates in the bam file (and in the cut profile), or the latter one if you wish to remove duplicates.
...
As you run create_script.py earlier, one of the script that has been generated is called `generate.footprinting.factor.specific.centipede.py` that is in the directory `macs2.narrow.aug18` or `macs2.narrow.aug18.dedup` within your workdir that you defined.
...
Note for **-p** option which is the motif scanning p-value, we recommend 0.0005, but for this example we will use 0.001 since GATA1 motif is quite short.

This Python script will generate a BASH script called `integrate.footprinting.GATA1.centipede.sh`, which can be run on a CUT&RUN experiment narrowPeak file (generated by MACS2). For example:
...
└----  fimo.GATA1.result
         └--- GATA1_D9_30min_S12_aligned_reads_peaks
                └---  fimo2.MA0035.2.GATA1
                        ...
                        └--- fimo.xml
```
The output files are `fimo.bed`, `fimo.cuts.freq.txt`, and `fimo.logratio.txt`, which contain information about the binding site locations, cut frequency matrix, and binding log odds for the provided GATA1 motif. 
