...
$macs2bin/macs2 callpeak -t $workdir/$dir/"$base_file".bam -g %s -f BAMPE -n $base_file --outdir $outdir -q 0.01 -B --SPMR 2> $logdir/"$base_file".dedup.macs2
""" % (macs2_org, macs2_org)
...
$extratoolsbin/bedGraphToBigWig "$base_file".sort.bdg $extrasettings/%s.chrom.sizes "$base_file".sorted.bw
...
Rscript run_centipede_parker.R $fimo_d/fimo.cuts.freq.txt $fimo_d/fimo.bed $fimo_d/fimo.png $mlen
done
"""
	outp.write(header + "\n")
	outp.write(scripts + "\n")
